...
class DemandeEntityRepository extends \Doctrine\ORM\EntityRepository
{
  public function filterDemande($filter, $em, $role, $idSalon, $column, $dir, $start, $length)
  {
    ...
    if ($role == 'ROLE_JURIDIQUE') {
      $query = $query->andWhere('d.service = :service')->setParameter('service', 'juridique');
    }
    ...
    if (in_array("coordinateur", $colFilter)) {
      // Récupération de l'id du coordinateur
      // via son nom + prénom
      $idPerso = $em->createQueryBuilder('p')
                    ->select('p.matricule as id')
                    ->from('ApiBundle:Personnel', 'p')
                    ->where("CONCAT(p.prenom, ' ', p.nom) = :name")
                    ->setParameter('name', $filter[5])->getQuery()->getSingleResult();

      // Récupération des ids des salons
      // du coordinateur
      $idSalon = $em->createQueryBuilder('s')
                    ->select('u.sage as id')
                    ->from('ApiBundle:PersonnelHasSalon', 's')
                    ->where("s.personnelMatricule = :id")
                    ->leftjoin('s.salonSage', 'u')
                    ->setParameter('id', $idPerso['id'])->getQuery()->getResult();
      ...
      $query = $query->andWhere('d.idSalon IN (:idSalon)')->setParameter('idSalon', $ids);
    }
    ...
    if (in_array("manager", $colFilter)) {
      ...
      $query = $query->leftjoin('d.user', 'e')
                     ->andwhere('e.matricule = :idPerso')
                     ->setParameter('idPerso', $idP);
    }
    ...
    if (in_array("type", $colFilter)) {
      ...
      $query = $query->leftjoin('d.demandeform', 't')
